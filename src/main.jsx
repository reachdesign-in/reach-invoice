...
function printDocument(doc, data, kind, notify = () => {}) {
  ...
  win.document.write(`<!doctype html><html><head><title>${kind} ${doc.number}</title><link rel="stylesheet" href="${icons}"><style>${printCss(data.settings.primaryColor)}</style></head><body>${renderPrintHtml(doc, data, kind)}</body></html>`);
  win.document.close();
...
async function createDocumentImageBlob(doc, data, kind) {
  ...
  const client = data.clients.find((item) => item.id === doc.clientId) || {};
  const pad = 72;
  const primary = data.settings.primaryColor || '#f3c910';
  const primaryDark = mixColor(primary, [0, 0, 0], 0.36);
  ...
  y += 12;
  drawContactLine(ctx, 'PHONE', data.settings.phone || '-', 520, y, primaryDark);
  drawContactLine(ctx, 'EMAIL', data.settings.email || '-', 700, y, primaryDark);
  drawContactLine(ctx, 'GSTIN', data.settings.gstNumber || 'Not provided', 520, y + 34, primaryDark);
...
function drawContactLine(ctx, label, value, x, y, accent = '#8a6800') {
  drawText(ctx, label, x, y, 56, 11, 800, accent);
...
function printCss(primaryColor = '#f3c910') {
  const primary = primaryColor || '#f3c910';
  const primaryDark = mixColor(primary, [0, 0, 0], 0.36);
  const primarySoft = mixColor(primary, [255, 255, 255], 0.72);
  const primarySoft2 = mixColor(primary, [255, 255, 255], 0.88);
  return `body{margin:0;font-family:"Google Sans","Product Sans",Inter,Arial,sans-serif;color:#202124}.sheet{max-width:980px;margin:auto;padding:34px 46px}.head{display:grid;grid-template-columns:1fr 1fr;align-items:center;gap:70px;padding:6px 8px 28px}.invoice-divider{position:relative;z-index:5;clear:both;width:100%;height:0;margin:8px 0 40px;border-top:2px solid ${primary}}.logo{width:220px;height:130px;display:grid;place-items:center;justify-self:start}.logo img{display:block;width:100%;height:100%;object-fit:contain}.company{width:330px;max-width:100%;justify-self:end}.company h1{margin:0 0 12px;font-size:30px;letter-spacing:0}.company p{margin:0;color:#5f6368}.address{display:grid;gap:3px;line-height:1.45;font-size:12px}.contact{display:grid;gap:8px;margin-top:14px}.contact span{display:grid;grid-template-columns:14px 42px minmax(0,1fr);align-items:center;gap:7px;font-size:11px}.contact i{color:${primaryDark};font-size:9px;text-align:center}.contact b{color:${primaryDark};font-size:8px;text-transform:uppercase;letter-spacing:.7px}.contact em{min-width:0;color:#303438;font-style:normal;overflow-wrap:anywhere}.meta{display:flex;justify-content:space-between;gap:40px;margin:0 8px 38px}.meta>div:last-child{min-width:210px;text-align:right}.meta b{font-size:15px}.meta p{margin:6px 0;color:#5f6368;font-size:13px}.meta:last-of-type{align-items:flex-end;margin-top:20px}table{width:calc(100% - 16px);margin:0 8px;border-collapse:collapse}th,td{padding:12px 10px;text-align:left;border-bottom:1px solid #dfe2e5}th{background:#fff;color:#5f6368;font-size:9px;text-transform:uppercase}td{font-size:13px}td small{display:block;margin-top:3px}.total{text-align:right;margin:18px 8px 14px}.total p{margin:9px 0;font-size:13px}.total h2{margin:16px 0 0;font-size:21px}.signature{border-top:2px solid #222;align-self:flex-end;padding-top:10px;min-width:200px;text-align:center;font-size:12px}.signature img{display:block;width:170px;max-height:58px;object-fit:contain;margin:-72px auto 10px}.signature span{display:block}.payment-history{margin:18px 8px;padding:12px 14px;border:1px solid ${primarySoft};border-radius:8px;background:${primarySoft2}}.payment-history div{display:flex;justify-content:space-between;gap:12px;padding-bottom:6px;border-bottom:1px solid ${primarySoft}}.payment-history div b{font-size:13px}.payment-history div span{color:#777b7f;font-size:11px}.payment-history p{margin:7px 0 0;display:flex;justify-content:space-between;gap:14px;color:#55595d;font-size:12px}.payment-history p strong{color:#202124;white-space:nowrap}@media print{button{display:none}.sheet{padding:24px 38px}}`;
}
