...
.preview-row-button { width: 100%; min-height: 58px; padding: 11px 4px; display: flex; align-items: center; justify-content: space-between; gap: 20px; border: 0; border-bottom: 1px solid #e5e7e9; border-radius: 0; background: transparent; color: inherit; text-align: left; }
.preview-row-button:hover { background: #fffaf0; color: #202124; }
.preview-row-button span { color: #202124; }
...
.preview-row-button > strong { font-size: 13px; white-space: nowrap; }
...
.modal { width: min(600px, 100%); max-height: calc(100vh - 48px); overflow-y: auto; overscroll-behavior: contain; background: #fff; border-radius: 8px; padding: 20px; box-shadow: 0 24px 70px rgba(0,0,0,.22); }
.modal.wide { width: min(1100px, 100%); }
.modal-head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; }
.modal-head h2 { margin: 0; font-size: 19px; }
.modal-actions { display: flex; justify-content: flex-end; gap: 8px; margin-top: 20px; }
.confirm-modal { width: min(430px, 100%); text-align: center; padding: 26px; }
.confirm-modal h2 { margin: 10px 0 8px; font-size: 22px; }
.confirm-modal p { margin: 0; color: #62666a; line-height: 1.55; }
.confirm-icon { width: 48px; height: 48px; display: grid; place-items: center; margin: 0 auto; color: #8a6800; background: #fff1a8; border-radius: 50%; }
.confirm-icon.danger { color: #a02828; background: #fff0f0; }
...
.payment-entry-field { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 8px; align-items: end; }
.full-paid-btn { min-height: 40px; white-space: nowrap; }
.payment-history { display: grid; gap: 8px; margin-top: 16px; padding: 14px; border: 1px solid #e8e0bc; border-radius: 8px; background: #fffdf4; }
.payment-history > div { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding-bottom: 6px; border-bottom: 1px solid #efe6c4; }
.payment-history > div strong { font-size: 14px; }
.payment-history > div span { color: #777b7f; font-size: 12px; }
.payment-history p { margin: 0; display: flex; align-items: center; justify-content: space-between; gap: 12px; color: #55595d; font-size: 13px; }
.payment-history p strong { color: #202124; white-space: nowrap; }
...
.trash-toolbar { display: flex; align-items: center; gap: 10px; margin-bottom: 16px; }
.trash-toolbar .search { flex: 1; }
.trash-list { display: grid; gap: 10px; }
.trash-card { display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 16px; background: #fff; border: 1px solid #e5e7e9; border-radius: 8px; box-shadow: 0 4px 14px rgba(25,25,25,.035); }
.trash-card > div:first-child { display: grid; gap: 4px; min-width: 0; }
.trash-card strong { font-size: 15px; }
.trash-card span, .trash-card small { color: #73777b; font-size: 12px; }
.trash-actions { display: flex; gap: 8px; flex-wrap: wrap; justify-content: flex-end; }
.admin-note { margin: -4px 0 16px; color: #696d72; }
.admin-tenant-list { display: grid; gap: 12px; }
.admin-tenant-card { display: grid; grid-template-columns: minmax(220px, 1fr) repeat(4, minmax(130px, 170px)); gap: 12px; align-items: end; padding: 14px; border: 1px solid #e5e7e9; border-radius: 8px; background: #fff; }
.admin-tenant-main { display: grid; gap: 4px; min-width: 0; }
.admin-tenant-main strong { font-size: 15px; }
.admin-tenant-main span, .admin-tenant-main small { color: #74787d; overflow-wrap: anywhere; }
.admin-tenant-card label { display: grid; gap: 6px; color: #60646a; font-size: 12px; font-weight: 700; }
.admin-tenant-card select, .admin-tenant-card input { width: 100%; height: 40px; padding: 0 10px; font-weight: 700; }
...
.logo-uploader > .signature-uploader { width: auto; height: auto; display: grid; place-items: stretch; border: 0; margin-top: 12px; color: inherit; }
.signature-uploader { display: grid; gap: 10px; }
.logo-uploader .signature-uploader > div { width: 170px; height: 82px; padding: 8px; background: linear-gradient(45deg, #fff 25%, #f3f4f5 25%, #f3f4f5 50%, #fff 50%, #fff 75%, #f3f4f5 75%); background-size: 16px 16px; border: 1px dashed #d8c26b; }
.logo-uploader .signature-uploader > div.has-signature { background-color: #fff; }
.signature-uploader img { width: 100%; height: 100%; object-fit: contain; border: 0; }
.signature-uploader span { color: #8a8f98; font-size: 12px; }
.signature-uploader small { width: 170px; color: #7a5b00; font-size: 11px; line-height: 1.35; }
.remove-signature { width: 170px; min-height: 32px; font-size: 11px; }
...
.signature img { display: block; width: 170px; max-height: 58px; object-fit: contain; margin: -70px auto 10px; }
...
@media (max-width: 800px) {
  ...
  .payment-entry-field { grid-template-columns: 1fr; }
...
@media (max-width: 520px) {
  ...
  .trash-toolbar, .trash-card { align-items: stretch; flex-direction: column; }
  .trash-actions { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); }
  .trash-actions button { width: 100%; }
  .admin-tenant-card { grid-template-columns: repeat(2, minmax(0, 1fr)); }
  .admin-tenant-main { grid-column: 1 / -1; }
  ...
  .report-metrics > span { min-height: 74px; padding: 11px; }
  .report-metrics strong { font-size: 16px; }
  .modal-backdrop { padding: 8px; }
  .modal { max-height: calc(100vh - 16px); padding: 15px; }
  .print-head, .print-meta { flex-direction: column; }
  .table-wrap { overflow: visible; }
  .table-wrap > table { display: none; }
  .mobile-doc-cards { display: grid; gap: 10px; padding: 10px; }
  .mobile-doc-card { display: grid; gap: 10px; padding: 13px; border: 1px solid #ece2b9; border-radius: 8px; background: #fff; box-shadow: 0 4px 12px rgba(25,25,25,.035); }
  .mobile-doc-head { display: flex; align-items: center; justify-content: space-between; gap: 10px; }
  .mobile-doc-head strong { font-size: 15px; }
  .mobile-doc-meta { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px; }
  .mobile-doc-meta span { display: grid; gap: 2px; min-width: 0; color: #202124; font-size: 12px; font-weight: 700; overflow-wrap: anywhere; }
  .mobile-doc-meta b { color: #777b7f; font-size: 9px; text-transform: uppercase; letter-spacing: .4px; }
  .mobile-doc-actions { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 7px; }
  .mobile-doc-actions button { min-height: 34px; padding: 0 6px; background: #f2f3f4; font-size: 11px; }
  .mobile-doc-actions .danger { background: #fff0f0; color: #a02828; border: 1px solid #ffc9c9; }
}
